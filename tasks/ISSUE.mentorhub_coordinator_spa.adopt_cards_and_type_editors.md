# Adopt spa_utils 0.5.0 cards and type-aligned editors (Coordinator SPA)

## Summary
Bump `@mentor-forge/mentorhub_spa_utils` to **0.5.0** and adopt `MhCard` / `CardGrid` / `DataCard` plus configurator-type editors for list and view/edit pages in `mentorhub_coordinator_spa`.

## Prerequisite
- `mentorhub_spa_utils` F015–F024 shipped and **`0.5.0` published** to CodeArtifact.

## Planning prompts (for `mentorhub_coordinator_spa` `tasks/_PLANNING.md`)
