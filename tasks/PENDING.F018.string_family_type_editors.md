...
**Type**: Feature  
**Depends On**: F017  
**Description**: Implement configurator-type-aligned string-family editors as thin derivatives of the F017 string base, including validation, view/edit modes, automation ids, and full-width layout inside cards.
...
Implement and export each editor below. Stick to Vuetify/Material Design defaults. Each editor must honor the shared contract (field key, visible, editable, automation id, AutoSave when editable).

### Planned component design (string family)

| Type | Component | Control | Validation / behavior |
|------|-----------|---------|------------------------|
| `word` | `WordEditor` | `v-text-field` | `^[^\s]{1,40}$`; single-line token (names, short ids) |
...
| `url` | `UrlEditor` | `v-text-field` | `^https?://[^\s]+$`; optional link affordance in view mode via `v-btn`/`a` if decided in F015 |
...
| `ip_address` | `IpAddressEditor` | `v-text-field` | IPv4/IPv6-aware rules; view mode plain text |
| `identifier` | `IdentifierEditor` | `v-text-field` | `^[0-9a-fA-F]{24}$`; **default editable=false** (ObjectIds rarely edited) |

Each component file should live under a clear folder (e.g. `src/components/editors/`) and be re-exported from `src/components/index.ts`.

Declarative usage target (prop names per F015):
...
- Unit tests per editor (or shared parameterized suites) covering validation, readonly/edit modes, and automation ids. Meet component coverage targets.
...
- `src/components/editors/WordEditor.vue` (paths may vary; keep consistent)
